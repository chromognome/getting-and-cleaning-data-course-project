...
5. From the data set in step 4, creates a second, independent tidy data set with the average of each variable for each activity and each subject
    - The mean of each variable — grouped by volunteer, activity performed, and originating data set — is calculated
...
The first three columns identify the volunteer carrying out the experiment (`subject`), the activity performed (`activity`), and which data set the summarised records came from (`dataset`). The remaining columns contain the means of each variable grouped by `subject`, `activity`, and `dataset`.  
